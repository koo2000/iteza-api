...
// HSVの設定
const int HSV_H_CENTER = 100;
const int HSV_H_RANGE = 25;
...
const int LINE_THREATHOLD_MIN = 300;
...
//		std::cerr << "line size = " << lines.size() << std::endl;
	}
    ...
    for (int i = 0; i < intersections.size(); i++) {
    	...
    	if (len < leftTopLen) {
    		for (int j = 0; j < intersections.size(); j++) {
    			...
    			if (abs(pos1.x - pos2.x) < 10 &&
    					abs(pos1.y - pos2.y) < 10) {
    				nearByPoints++;
    			}
    			if (nearByPoints >= 5) {
    				break;
